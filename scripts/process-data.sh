...
function get_url() {
    # echo "https://www.volksgezondheidenzorg.info/sites/default/files/map/detail_data/klik_corona$(date +%d%m%Y)_2.csv"
    echo https://www.volksgezondheidenzorg.info/sites/default/files/map/detail_data/klik_corona10032020_2.csv
...
TOTAL=$(ogrinfo data/corona_nl.gpkg -sql "select sum(aantal) as total from gemeenten_corona"  | grep "total " | cut -d= -f 2)
echo "{\"date_data\": \"$DATE_DATA\",\"url\": \"$URL_DATA\", \"total_infections\":\"$TOTAL\"}" > webapp/updated.json
...
python classify.py ../data/corona_nl.gpkg 5 --layer gemeenten_corona --attribute aantal > ../webapp/classes.json